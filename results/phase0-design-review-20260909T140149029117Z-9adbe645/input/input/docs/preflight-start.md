...
The output directory receives RUN-RECORD.json and START-REVIEW.txt. The script also prints the complete handoff automatically: the two directories and their intended permissions, a clearly delimited copy/paste block identical to START-REVIEW.txt, and the expected report path. Follow that terminal output; opening a separate file is no longer a required step. Supply only the pack, that output directory and the generated startup instruction to the reviewer. The actual run record and startup instruction are explicitly authorized administrative metadata, not additional design evidence. Do not attach this document, the main repository, the helper's source or the presentation journal to the independent designer.

## Reprint an already prepared handoff

```bash
python3 scripts/prepare-review-run.py --show-handoff ../RustyBun-review-01-output
```

This mode only reads and prints. It does not create files, modify timestamps, authorize a blocked run, or restart a completed/started task. It verifies the pack against the recorded manifest hash and checks that the saved startup instruction matches the record. The record must still be PREPARED, with no execution timestamps or design report. A stale path, changed input/instruction or used record is an error, not permission to overwrite anything. Existing records generated by the original helper are supported. To change container paths, update both the record and saved instruction explicitly; the tool does not silently remap them. Printing the handoff does not mount folders or configure the agent environment.
